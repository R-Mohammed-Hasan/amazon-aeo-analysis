...
const RoastPanel = memo(function RoastPanel({
  ...
  return (
    <section
      className="rounded-2xl border border-[var(--border)] bg-[var(--card)] p-5 animate-fade-up"
...
const AEOPanel = memo(function AEOPanel({ aeo }: { aeo: AEOResult | null }) {
  return (
    <section
      className="rounded-2xl border border-[var(--border)] bg-[var(--card)] p-5 animate-fade-up delay-1"
      style={{ boxShadow: "inset 0 1px 0 rgba(99,102,241,0.06)" }}
    >
      {aeo ? <AEOReport result={aeo} /> : <AEOSkeleton />}
    </section>
  );
});
...
function ResultsContent() {
  ...
  const [fixedCopy, setFixedCopy] = useState<{
    issue: string;
    fixed: string;
  } | null>(null);
  ...
  useEffect(() => {
    ...
    async function run() {
      try {
        ...
        if ((listingData as { error?: string }).error)
          throw new Error((listingData as { error?: string }).error);
  ...
  const handleFix = useCallback(
    async (issue: RoastIssue, key: string) => {
    ...
    return (
      <main className="min-h-screen flex flex-col items-center justify-center gap-8 px-4">
        <div className="relative flex items-center justify-center w-16 h-16">
          <div
            className="absolute inset-0 rounded-full border border-[var(--accent)]/20 animate-ping"
            style={{ animationDuration: "2s" }}
          />
          <div
            className="w-12 h-12 rounded-full border-2 border-[var(--border-lit)] border-t-[var(--accent)] animate-spin"
            style={{ animationDuration: "1s" }}
          />
          <span className="absolute text-xs font-extrabold text-[var(--accent)]">
            IQ
          </span>
        </div>
        <div className="text-center">
          <p className="font-bold text-[var(--fg)] text-lg tracking-tight mb-1">
            Scraping listing…
          </p>
          <p className="text-sm text-[var(--muted)] font-normal">
            Fetching title, bullets &amp; images via Firecrawl
          </p>
        </div>
  ...
  if (error) {
    return (
      <main className="min-h-screen flex flex-col items-center justify-center gap-5 px-4">
        <div className="w-10 h-10 rounded-full bg-[var(--critical-bg)] border border-[var(--critical)]/30 flex items-center justify-center text-[var(--critical)] text-xl">
          ✕
        </div>
        <div className="text-center">
          <p className="font-bold text-[var(--fg)] mb-1">Analysis failed</p>
          <p className="text-sm text-[var(--muted)] max-w-xs font-normal">
            {error}
          </p>
        </div>
        <button
    ...
    <main className="min-h-screen px-4 pb-20 max-w-5xl mx-auto">
      ...
      >
        <div className="flex items-center justify-between gap-4 flex-wrap">
          ...
          >
            <span className="text-[var(--muted)] group-hover:-translate-x-0.5 transition-transform">
              ←
            </span>
      ...
      <div className="mb-6 animate-fade-up">
        <h1 className="text-2xl font-extrabold text-[var(--fg)] leading-snug mb-3 max-w-3xl">
          {listing.title}
        </h1>
        ...
        <div className="mt-6 rounded-2xl border border-[var(--border)] bg-[var(--card)] p-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 animate-fade-up delay-2">
          <div className="flex items-start gap-3">
            <div
              ...
              style={{
                background: "linear-gradient(135deg, #7C6CF8, #F59E0B)",
                opacity: 0.9,
              }}
            >
              ✦
            ...
            <div>
              <p className="font-bold text-[var(--fg)] text-sm">
                Copy fixes need matching visuals
              </p>
              <p className="text-xs text-[var(--muted)] font-normal mt-0.5 max-w-sm">
                Better listing images convert 20–40% more than copy alone. Pixii
                AI generates studio-quality lifestyle photos &amp; infographics
                from your product images.
              </p>
            </div>
          </div>
          <a
            ...
            style={{
              background: "linear-gradient(135deg, #7C6CF8, #9D6CF8)",
              color: "#fff",
            }}
          >
            Try Pixii →
...
function Chip({
  ...
  return (
    <span
      style={style}
      className="text-xs bg-[var(--faint)] border border-[var(--border)] text-[var(--fg)] px-2.5 py-1 rounded-lg font-normal"
    >
      {children}
    </span>
  );
